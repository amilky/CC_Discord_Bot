...
class Channel():
    async def send(str=None, file=None):
        if str is not None:
            print(str)
        if file is not None:
            new_file=''.join(random.choice(string.ascii_letters) for x in
                             range(10))+".png"
            print(new_file)
            input("WAIT4")
...
async def mainloop():
    while True:
        ...
        if "p" == commandy:
            ...
            importlib.reload(sys.modules['pointsdisplay.fullpointsdisplay'])
            ...
            rsn = cl[1]
            await bot.points(ctx, rsn)
        elif "!fullpoints" in commandy:
            ctx = Context()
            cl = commandy.split(" ")
            rsn = cl[1]
            await bot.full_points(ctx, rsn)
